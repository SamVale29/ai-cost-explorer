...
if (failures.length === 0) {
  console.log(
    'All source URLs responded or explicitly rejected automated probing. No data was mutated.',
  );
} else {
  console.warn(
    `Unavailable source URLs: ${failures.length}. This is a review signal, not an automatic data edit.`,
  );
  for (const failure of failures)
    console.warn(`- ${failure.source.publisher}: ${failure.source.url} (${failure.status})`);
}
