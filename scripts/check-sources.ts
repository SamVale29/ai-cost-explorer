...
const invalid = sources.filter((source) => {
  try { return !['http:', 'https:'].includes(new URL(source.url).protocol); } catch { return true; }
});
if (invalid.length) {
...
if (process.env.CHECK_SOURCES_NETWORK === '1') {
  const results = await Promise.all(sources.map(async (source) => {
    try {
      const response = await fetch(source.url, { method: 'HEAD', signal: AbortSignal.timeout(8_000) });
  ...
  if (failures.length) {
    console.warn(`Network source check found ${failures.length} unavailable URL(s); data is not mutated.`);
