...
import { calculateCatalogHealth, type CatalogHealth } from '../lib/catalog-health';
    ...
    <div className="app-shell">
      ...
      <aside className={`sidebar ${mobileOpen ? 'sidebar-open' : ''}`} aria-label="Primary navigation">
        ...
        <div className="sidebar-bottom">
          ...
          <button className="theme-toggle" type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} theme`}><span>{theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}</span>{theme === 'dark' ? 'Light theme' : 'Dark theme'}</button>
        </div>
      </aside>
      {mobileOpen && <button className="mobile-backdrop" type="button" onClick={() => setMobileOpen(false)} aria-label="Close navigation" />}
      <main id="main-content" className="main-content" tabIndex={-1}>
        <header className="topbar">
          <button className="mobile-menu" type="button" onClick={() => setMobileOpen(true)} aria-label="Open navigation" aria-expanded={mobileOpen} aria-controls="primary-navigation"><Menu size={20} /></button>
          <div className="breadcrumbs"><span>AI COST EXPLORER</span><ChevronDown size={13} /><strong>{pageTitle}</strong></div>
          <div className="topbar-actions"><span className="updated-label"><span className="pulse-dot" />Catalog checked {formatDate(catalog.dataAsOf)}</span><button className="icon-button" type="button" onClick={toggleTheme} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} theme`}>{theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}</button><a className="icon-button" href={GITHUB_URL} target="_blank" rel="noreferrer" aria-label="Open GitHub"><Github size={17} /></a></div>
        </header>
...
function CatalogHealthPanel({ health }: { health: CatalogHealth }) {
  return <section className="catalog-health-panel table-card" aria-labelledby="catalog-health-title"><div className="catalog-health-heading"><div><span className="eyebrow">CATALOG HEALTH</span><h2 id="catalog-health-title">Coverage you can inspect.</h2><p>Percentages describe known fields in the reviewed public snapshot, not provider quality.</p></div><a className="text-link" href={`${GITHUB_URL}/blob/main/public/data/catalog-health-v1.json`} target="_blank" rel="noreferrer">Open report <ExternalLink size={13} /></a></div><div className="catalog-health-grid"><div><strong>{health.coverage.offersWithStandardInputPrice}%</strong><span>Offers with input price</span></div><div><strong>{health.coverage.offersWithStandardOutputPrice}%</strong><span>Offers with output price</span></div><div><strong>{health.coverage.modelsWithContextWindow}%</strong><span>Models with context</span></div><div><strong>{health.coverage.modelsWithVerifiedCapability}%</strong><span>Models with a verified capability</span></div></div><div className="catalog-health-footer"><span>{health.freshness.freshSources}/{health.totals.sources} sources fresh as of {formatDate(health.dataAsOf)}</span><Badge tone={health.benchmarkStatus === 'empty' ? 'gold' : 'mint'}>{health.benchmarkStatus === 'empty' ? 'Benchmarks empty by design' : 'Benchmarks available'}</Badge></div></section>;
}
...
function FilterChip({ label, onRemove }: { label: string; onRemove: () => void }) { return <span className="filter-chip">{label}<button type="button" onClick={onRemove} aria-label={`Remove ${label} filter`}><X size={12} /></button></span>; }
function SortableHeader({ label, sortKey, sort, setSort }: { label: string; sortKey: string; sort: { key: string; direction: 'asc' | 'desc' }; setSort: (value: { key: string; direction: 'asc' | 'desc' }) => void }) { const active = sort.key === sortKey; const direction = active ? sort.direction : 'asc'; return <th><button className={`sort-header ${active ? 'active' : ''}`} type="button" onClick={() => setSort({ key: sortKey, direction: active && sort.direction === 'asc' ? 'desc' : 'asc' })} aria-label={`Sort by ${label}, ${direction === 'asc' ? 'ascending' : 'descending'}`} aria-pressed={active}>{label}<ArrowDownUp size={12} /></button></th>; }
